...
questions using Gemma4 via the local Ollama API.
...
def ask_gemma4(question: str, from_peer: str, model: str) -> str:
...
def run(model: str, socket_path: str) -> None:
    ...
    while True:
        ...
        response = ask_gemma4(content, from_peer, model)
        print(f"[muse] Gemma4 says: {response}\n", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Foreman Muse bridge (Gemma4 via Ollama)")
