...
adversarial plan/result review using Google Gemini via the google-genai SDK.
...
def ask_gemini(question: str, from_peer: str, chat) -> str:
    prompt = f'[From {from_peer}]: {question}'
    try:
        response = chat.send_message(prompt)
        return response.text.strip()
    except Exception as e:
        return f"[Dissenter unavailable — Gemini error: {e}]"
...
def run(model: str, socket_path: str) -> None:
    ...
    while True:
        ...
        response = ask_gemini(content, from_peer, chats[from_peer])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Foreman Dissenter bridge (Gemini via google-genai SDK)")
    parser.add_argument("--model", default=DEFAULT_MODEL, help="Gemini model name (default: gemini-3.1-pro)")
