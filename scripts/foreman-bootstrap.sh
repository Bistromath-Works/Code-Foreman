...
#
# Model configuration:
#   - Edit the MODEL values for "dissenter" and "muse" below to match
#     your chosen providers. All other roles use standard Claude models.
#   - The Dissenter bridge (foreman-dissenter-bridge.py) requires a
#     matching API key in .env.local (see .env.local.example).
#   - The Muse bridge (foreman-muse-bridge.py) defaults to a local
#     Ollama server. Update OLLAMA_URL in the bridge script if needed.
...
case "$ROLE" in
  orchestrator)
    ...
    ;;
  dissenter)
    # Set this to the model name your chosen API provider expects.
    # e.g. "gemini-2.0-flash", "gpt-4o", "claude-opus-4-5", etc.
    MODEL="your-dissenter-model-here"
  ...
  muse)
    ...
    # Must be a model distinct from the Claude crew — the value of
    # the Muse comes from genuinely different weights/reasoning.
    # e.g. "gemma3", "llama3.2", "mistral", "phi4", etc.
    MODEL="your-muse-model-here"
...
#!/usr/bin/env zsh
cd $q_cwd
echo "[muse] Starting model bridge..."
exec python3 $q_bridge --model $q_model
SCRIPT
    elif [ "$ROLE" = "dissenter" ]; then
      # Dissenter runs as a Python bridge talking directly to the relay hub + an external model API.
      # See foreman-dissenter-bridge.py for SDK requirements and .env.local.example for API key setup.
      local q_bridge
      q_bridge="$(printf '%q' "$FOREMAN_DIR/scripts/foreman-dissenter-bridge.py")"
      cat > "$tmpscript" <<SCRIPT
#!/usr/bin/env zsh
cd $q_cwd
echo "[dissenter] Starting model bridge..."
exec python3 $q_bridge --model $q_model
SCRIPT
    else
